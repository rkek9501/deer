...
appRouter.post("/tag", async (req, res) => {
...
appRouter.put("/tag", async (req, res) => {
  const { tagid, color, name } = req.body;
  try {
    if (!isValidHex(color)) return res.status(200).send({ result: false, message: "color 값이 올바르지 않습니다." });
    const result = await models.sequelize.transaction(async (t: Transaction) => {
      return await tags.findOne({ where: { id: tagid } }, { transaction: t }).then(async (tag: any) => {
...
appRouter.delete("/tag", async (req, res) => {
  const { id } = req.query;
  try {
    const result = await models.sequelize.transaction(async (t: Transaction) => {
